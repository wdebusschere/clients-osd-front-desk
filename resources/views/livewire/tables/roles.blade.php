<div>
    ...
    @else
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        @include('livewire.tables.partials.col-header', ['title' => trans_choice('app.permissions', 0)])
                        @include('livewire.tables.partials.col-header', ['class' => 'w-0'])
                    </tr>
                </thead>
                <tbody>
                    @foreach($roles as $role)
                        <tr wire:key="dt-row-key-{{ $role->id }}">
                            <td>{{ $role->id }}</td>
                            <td>{{ $role->name }}</td>
                            <td>
                                <x-ui.badge>
        ...
        <x-ui.pagination-controls :collection="$roles" class="mt-4"/>
    @endif
</div>
